...
library BitMask {
    ...
    function _isSet(uint256 mask_, uint8 index_) internal view returns (bool) {
    ...
    function _unset(uint256 mask_, uint256 index_) internal view returns(uint256) {
        uint256 indexMask = 1 << (index_ & 0xff);
        return mask_ &= ~indexMask;
    }

    /// Gets the indexes of the set bits in the mask as an array
    function _getSetIndexes(uint256 mask_) internal view returns (uint8[] memory) {
    ...
    function _convertToMask(uint8[] memory indexes_) internal view returns (uint256) {
        uint256 mask = 0;
