...
import { IIPOrg } from "contracts/interfaces/ip-assets/IIPOrg.sol";
import { ICollectModule } from "contracts/interfaces/modules/collect/ICollectModule.sol";
import { IPAsset } from "contracts/lib/IPAsset.sol";
import { IPOrgDataManager } from "./storage/IPOrgDataManager.sol";
    ...
    OwnableUpgradeable
{
    ...
    /// Creates a new IPAsset, and assigns licenses (rights) to it, according to the IPOrg
    ...
    /// @dev reverts if LicensingModule is not configured for the IPOrg.
    ...
    function createIpAsset(IPAsset.CreateIpAssetParams calldata params_) public returns (uint256, uint256) {
        ...
        IPOrgStorage storage $ = _getIPOrgStorage();
    ...
    function _mintBlock(
        ...
        if (nextId > IPAsset._lastId(ipAssetType_)) revert Errors.IPOrg_IdOverBounds();
        IPOrgStorage storage $ = _getIPOrgStorage();
